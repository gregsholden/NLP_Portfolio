...
Inputs:  FOMC Minutes transcripts are the primary inputs to the NLP_Analysis_Final.ipynb notebook in the NLP_Analsysis folder.  This info can be accessed in two ways:
...
Outputs:   The NLP_Analysis_Final.ipynb notebook outputs 3 separate .csv files which can be used in the Portfolio Optimization models: (a) one containing vectors from the Universal Sentence Encoder (2) a second file containing results of the Named Entity Recognition modeling, and (3) a third file containing the results of all other NLP analyses.
